...
{
    public class EmployeesRepository : IRepository<Employees, string>
    {
        public IConfiguration _configuration;
        private readonly AppDbContext context;
        public EmployeesRepository(IConfiguration configuration, AppDbContext context)
        {
        ...
        public IEnumerable<Employees> Get()
        {
            using (SqlConnection connection = new SqlConnection(_configuration["ConnectionStrings:APISistemAbsensi"]))
            {
                ...
                var res = connection.Query<Employees>(spName, parameters, commandType: CommandType.StoredProcedure);
                return res;
            }
        }
        ...
        public virtual int RegisterVM(RegisterVM registerVM)
        {
            using (SqlConnection connection = new SqlConnection(_configuration["ConnectionStrings:API"]))
            {
                ...
                parameters.Add("@Manger_Id", registerVM.Manager_Id);
                parameters.Add("@DepartmentId", registerVM.DepartmentId);
